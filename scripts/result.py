...
def parse_file(file_name):
    ...
    while True:
        ...
        if not line:
            break
        arr = line.split()
        '''size (bytes), fct (seconds), timeouts, packetloss'''
        if len(arr) >= 3:
            size = float(arr[0])
            fct = float(arr[1]) * 1000000 #second to us
            timeouts = int(arr[2])
            packetloss = int(arr[3])
            results.append([size, fct, timeouts, packetloss])
    f.close()
    return results
...
def print_result(results):
    # (0, 10KB)
    small1 = filter(lambda x: x[0] < 10 * 1024, results)
    # (10, 100KB)
    small2 = filter(lambda x: 10 * 1024 <= x[0] < 100 * 1024, results)
    # (0, 100KB)
    ...
    print '%d flows total packet loss: %d' % (len(results), sum([x[3] for x in results]))
    #print '%d flows (0, 10KB) average completion time: %d us' % (len(small1), average_fct_result(small1))
    #print '%d flows (0, 10KB) 99th percentile completion time: %d us' % (len(small1), cdf_fct_result(small1, 0.99))
    #print '%d flows (10KB, 100KB) average completion time: %d us' % (len(small2), average_fct_result(small2))
    #print '%d flows (10KB, 100KB) 99th percentile completion time: %d us' % (len(small2), cdf_fct_result(small2, 0.99))
    ...
    print '%d flows (0, 100KB) 99.9th percentile completion time: %d us' % (len(small), cdf_fct_result(small, 0.999))
